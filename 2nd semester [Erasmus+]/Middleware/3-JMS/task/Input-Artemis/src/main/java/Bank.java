...
public class Bank implements MessageListener {
	...
	//	initial balance for newly created accounts
	public static final int INITIAL_BALANCE = 10000;

	//	order type "check balance"
	public static final int ORDER_TYPE_CHECK = 2;
	...
	private void processTextMessage(TextMessage txtMsg) throws JMSException {
		...
		if (NEW_ACCOUNT_MSG.equals(txtMsg.getText())) {
			...
			if (clientAccounts.get(clientName) != null) {
				...
				// also initialize balance
				accountsBalances.put(accountNumber, INITIAL_BALANCE);
			}
			
			System.out.println("Connected client " + clientName + " with account " + accountNumber + " (initial balance $" + INITIAL_BALANCE + ")");
	...
	private void processMapMessage(MapMessage mapMsg) throws JMSException {
		...
		if (order == ORDER_TYPE_SEND) {
			...
			// thread-safe account balance update
			accountsBalances.compute(clientAccount, (accountKey, balanceVal) -> {
				if (balanceVal >= amount) {
					// free goods included as well
					
					System.out.println("Transferring $" + amount + " from account " + clientAccount + " to account " + destAccount);

					// update seller account balance: add amount
					accountsBalances.put(destAccount, accountsBalances.get(destAccount) + amount);

					// set amount of money transfered
					try {
						reportMsg.setInt(AMOUNT_KEY, amount);
					} catch (JMSException jmse) {}
					
					// update buyer account balance: subtract amount
					return balanceVal - amount;
				}
				else {
					System.out.println("Declined transfer of $" + amount + " from account " + clientAccount + " to account " + destAccount + " due to insufficient balance");
		...
		else if (order == ORDER_TYPE_CHECK) {
			// get client's name
			String clientName = mapMsg.getStringProperty(Client.CLIENT_NAME_PROPERTY);
			...
			int clientAccountBalance = accountsBalances.get(clientAccount);

			// get client Destination
			Destination clientDestination = mapMsg.getJMSReplyTo();

			// create reply message for the client
			TextMessage replyMsg = bankSession.createTextMessage(String.valueOf(clientAccountBalance));

			// send reply message to client's destination
			bankSender.send(clientDestination, replyMsg);
		}
		else {
