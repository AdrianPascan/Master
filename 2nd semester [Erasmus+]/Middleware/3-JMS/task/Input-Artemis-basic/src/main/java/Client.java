...
public class Client {
	...
	private void buy() throws IOException, JMSException {
		...
		if (confirmationMsg instanceof TextMessage) {
			TextMessage confirmationTextMsg = (TextMessage) confirmationMsg;
			String confirmationGoodsName = confirmationTextMsg.getText();
		...
		if (msg instanceof MapMessage) {
			...
			if (cmd == Bank.REPORT_TYPE_RECEIVED) {
				...
				if (amount >= g.price) {
					...
					TextMessage confirmationMsg = clientSession.createTextMessage(g.name);
